...
std::vector<CTxOut> GetBetPayoutsDebugOrig(int height, UniValue &debugObject)
{
    ...
    int nScanStartHeight = height - Params().BetBlocksIndexTimespan() > Params().BetStartHeight() ? height - Params().BetBlocksIndexTimespan() : Params().BetStartHeight();
    int nLastScannedBlock = 0;

    debugObject.push_back(Pair("nCurrentHeight", nCurrentHeightUnused));
    debugObject.push_back(Pair("height", height));
    debugObject.push_back(Pair("nScanStartHeight", nScanStartHeight));

    UniValue uvResults(UniValue::VARR);
    // Traverse the blockchain for an event to match a result and all the bets on a result.
    for (const auto& result : results) {
        UniValue uvResult(UniValue::VOBJ);
        uvResult.push_back(Pair("nEventID", (uint64_t)result.nEventId));
        uvResult.push_back(Pair("nResultType", (uint64_t)result.nResultType));
        uvResult.push_back(Pair("nHomeScore", (uint64_t)result.nHomeScore));
        uvResult.push_back(Pair("nAwayScore", (uint64_t)result.nAwayScore));

        // Look back the chain 14 days for any events and bets.
        CBlockIndex *BlocksIndex = chainActive[nScanStartHeight];
        ...
        uvResult.push_back(Pair("nMoneyLineResult", nMoneylineResult));

        UniValue uvResultsBlocks(UniValue::VARR);
        // Traverse the block chain to find events and bets.
        while (BlocksIndex && BlocksIndex->nHeight < height) {
            CBlock block;
...
//                        LogPrint
            ...
            nLastScannedBlock = BlocksIndex->nHeight;
